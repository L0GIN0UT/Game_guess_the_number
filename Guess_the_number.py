# Компьютер загадыват случайное число от 1 до 100
# Пользователю предлагают угадать
# Если пользователь угадал, программа сообщает о победе.
# Если пользователь ввел неверное число, программа дает ему подсказку:
# Введеное число больще или меньше загаданного
...
while count != res_count:
    if val_in > hidden_numer:
        ...
        break
    print(f'Осталось {res_count} попыток')
    val_in = input_number_cycle()
if count > res_count:
